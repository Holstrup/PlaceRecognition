...
class IoUGeneralizedContrastiveLoss(nn.Module):
    r"""CONTRASTIVELOSS layer that computes contrastive loss for a batch of images:
        Q query tuples, each packed in the form of (q,p,n1,..nN)
    ...
    def __init__(self, margin=0.7, eps=1e-6, gpsmargin=25, beta=1.0):
        super(IoUGeneralizedContrastiveLoss, self).__init__()
        self.margin = margin
        self.eps = eps
        self.mse_loss = 0

    def forward(self, x, label, gps=[]):
        loss, smoothing_factor = LF.iou_generalized_contrastive_loss(x, label, gps, margin=self.margin, eps=1e-6)
        self.mse_loss = smoothing_factor
        return loss
...
class IoUGeneralizedMSELoss(nn.Module):
    r"""CONTRASTIVELOSS layer that computes contrastive loss for a batch of images:
        Q query tuples, each packed in the form of (q,p,n1,..nN)
    ...
    def __init__(self, margin=0.7, eps=1e-6, gpsmargin=25, beta=1.0):
        super(IoUGeneralizedMSELoss, self).__init__()
        self.margin = margin
        self.eps = eps
        self.mse_loss = 0

    def forward(self, x, label, gps=[]):
        loss, smoothing_factor = LF.iou_mse_contrastive_loss(x, label, gps, margin=self.margin, eps=1e-6)
        self.mse_loss = smoothing_factor
        return loss
